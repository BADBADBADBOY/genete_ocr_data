...
def get_rotate_coord(img,angle):
...
class Block:
    def __init__(self, img: Image.Image, inner_x=0, inner_y=0, margin=0, rotate_angle=0):
        ...
        self.coord_text = get_rotate_coord(img,rotate_angle)
...
class Layout:
    ...
    def collect_block_fragment(self):
        ...
        for block in self.get_all_block_list():
            ...
            fragment_text_coord = block.coord_text.reshape(8).tolist()
            fragment_data = block.get_data()
            orientation = block.get_orientation()
            item = {
                "img": fragment_img,
                "box": fragment_box,
                "coord":fragment_text_coord,
                "data": fragment_data,
    ...
    def dump(self):
        from service.base import get_data_dir, get_pic_dir
